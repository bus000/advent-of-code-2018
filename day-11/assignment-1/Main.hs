...
newtype Square a = Square [(a, a)] deriving (Show)

gridValue :: Grid -> (Int, Int) -> Int
gridValue (Grid serial) (x, y) = hundreds - 5
  where
    rackId = x + 10
    powerLevel = ((rackId * y) + serial) * rackId
    hundreds = powerLevel `div` 100 `mod` 10

squareValue :: Grid -> Square Int -> Int
squareValue grid (Square positions) = sum . map (gridValue grid) $ positions

square :: Integral a => (a, a) -> Square a
square (x, y) = Square $ (,) <$> [x..x+2] <*> [y..y+2]

squares :: [Square Int]
squares = map square $ (,) <$> [1..300-3] <*> [1..300-3]

getPositions :: Square a -> [(a, a)]
getPositions (Square positions) = positions

parseInput :: LText -> Either P.ParseError Grid
parseInput = P.parse (parseGrid <* P.eof) ""

parseGrid :: P.Parsec LText () Grid

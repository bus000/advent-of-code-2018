...
handleInput :: MonadIO m => Int -> m ()
handleInput n = print . S.take 10 . S.drop n . runSimulation . (+ 10) $ n
...
data State = State (S.Seq Int) Elf Elf

runSimulation :: Int -> S.Seq Int
runSimulation n = result
  where
    State result _ _ = M.execState (runSteps n) initialState
    initialState = State initialSeq elf1 elf2
    initialSeq = S.fromList [3, 7]
    elf1 = Elf 3 (S.singleton 7)
    elf2 = Elf 7 S.empty

runSteps :: M.MonadState State m => Int -> m ()
runSteps steps = replicateM_ steps $ generateRecipes >> moveElfs

generateRecipes :: M.MonadState State m => m ()
generateRecipes = do
    State recipes (Elf v1 rest1) (Elf v2 rest2) <- M.get

    let newVals = S.fromList $ digits (v1 + v2)
        recipes' = recipes >< newVals
        elf1' = Elf v1 (rest1 >< newVals)
        elf2' = Elf v2 (rest2 >< newVals)

    M.put (State recipes' elf1' elf2')

moveElfs :: M.MonadState State m => m ()
moveElfs = do
    State recipes elf1 elf2 <- M.get

    let elf1' = move recipes elf1
        elf2' = move recipes elf2

    M.put (State recipes elf1' elf2')

move :: S.Seq Int -> Elf -> Elf
move recipes (Elf c rest)
    | c >= S.length rest = move recipes $ Elf (c - S.length rest) recipes
    | otherwise = Elf c' rest'
  where
    c' :< rest' = S.viewl . S.drop c $ rest
